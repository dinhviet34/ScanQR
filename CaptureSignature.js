...
class CaptureSignature extends Component {
  ...
  func_render_quota = () => {
    ...
    if (this.state.usequota !== "") {
      return (
        <View>
          <ProgressBar progress={phantram / 100} color="#009FBD" style={{ height: 10, }} />
        </View>
      )
    }
      ...
      if (sizeInMB < 5) {
        if (this.state.valuecompess !== null) {
          ...
          if (this.state.valueresize !== null) {
            resi = parseFloat(this.state.valueresize);
          }
          var comp = parseFloat(this.state.valuecompess);
          const manipResult = await ImageManipulator.manipulateAsync(
            photo.assets[0].uri,
            [{ resize: { width: photo.assets[0].width * resi, height: photo.assets[0].height * resi } }],
            { compress: comp, format: ImageManipulator.SaveFormat.JPEG }
          );
        ...
        Alert.alert("Thông báo", "Dung lượng cho phép là 5MB đã vượt quá dung lượng cho phép, xóa bớt ảnh hoặc thoát chương trình thự hiện lại từ đầu")
      }
    ...
    if (status !== 'granted') {
      ...
      if (sizeInMB <= 5) {
        if (this.state.valuecompess !== null) {
          ...
          var photo = await this.camera.takePictureAsync();
          var comp = parseInt(this.state.valuecompess);
          var resi = 1;
          if (this.state.valueresize !== null) {
            resi = parseFloat(this.state.valueresize);
          }
          ...
          const manipResult = await ImageManipulator.manipulateAsync(
            photo.uri,
            [{ resize: { width: photo.width * resi, height: photo.height * resi } }],
            { format: ImageManipulator.SaveFormat.JPEG }
          );
    ...
    if (this.state.renderimage === true) {
      return (
        <View style={styles.containerMainontop}>
          ...
          <View style={{ justifyContent: 'center', alignContent: 'center', backgroundColor: '#ddd', flex: 1 }}>
            ...
            <ImageEditor
              ...
              onEditingComplete={async (image) => {
                ...
                let listimagesuri = this.state.listimagesuri;
                listimages.push(image.uri);
  ...
  func_remove_selected_image = async (value) => {
    let listimages = this.state.listimages;
    var index = listimages.indexOf(value);
    console.log(value);

    let fileInfo = await FileSystem.getInfoAsync(value);
    let sizefile = fileInfo.size;
    let totalsize = this.state.totalsize;
    totalsize = totalsize - sizefile;
    this.setState({ totalsize: totalsize });
